...
const Products = () => {
  ...
  <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
    ...
    {loading ? filterData.map((product) => (
        <div key={product.id}>
        <li>
        <Link class="flex flex-col group bg-white border shadow-sm rounded-xl overflow-hidden hover:shadow-lg transition dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]" to={`/product-detail/${encodeURIComponent(JSON.stringify(product))}`}>
  <div class="relative pt-[50%] sm:pt-[60%] lg:pt-[80%] rounded-t-xl overflow-hidden">
    <img class="w-full h-full absolute top-0 left-0 object-cover group-hover:scale-105 transition-transform duration-500 ease-in-out rounded-t-xl" src={product.thumbnail} alt={product.title} />
  </div>
